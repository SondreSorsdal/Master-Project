...
Aphid_shoots_plum  <- Aphid_shoots_plum  %>%
  ...
  Aphid_shoots_time_series <- Aphid_shoots_plum %>% 
  mutate(treatment=dplyr::recode(treatment, '1' = "Control", '2' = "FS14", '4' = "Control+Aphid", '5' = "FS14+Aphid", '6' = "FS1", '7' = "FS1+Aphid" )) 

#Below is the script for manipulating the data for use in Minitab for creating the time series graph. 

timeseries_aphid_shoots_plum <- data.frame(Aphid_shoots_time_series %>%
...
p1 <- ggplot(timeseries_aphid_shoots_plum) +
...
#write.xlsx(Aphid_shoots_plum, "spray_frass_plum")
...
tab_df(aphid_shoots_mean,
       title = "Table 3: Descriptive Statistics of Aphid Shoots on Plum trees in relation to Treatment")
...
#Calculating p-values for signficance of average differences, using bonferroni method
options(digits = 3,scipen = 999)
dun_test <- dunnTest(aphid_shoots~treatment,Aphid_shoots_plum,method="bonferroni", list = TRUE)
dun_test

#creating vector of p.adjusted from dun_test so that it fits the arrangement of Mean_Diff
P.adj <- c(1.000000000000000000000000000000, 0.000000000000000000530110704691, 0.000000000000000000000000000472, 0.000000000000000203179636004859, 0.000000000000000014024194608921, 0.000000000000000007626269892446, 0.000000000000000000000000012160, 0.000000000000002016630854689484, 0.000000000000000181981783235854,
0.648330939055542621041183792840,	1.000000000000000000000000000000, 1.000000000000000000000000000000, 0.961530744426840500160835745191,	0.259475509665134884684789540188,	1.000000000000000000000000000000)
...
#Post_Hoc <- Post_Hoc %>% 
 # filter(Pairs %in% c("2-1", "6-1", "6-2", "5-4", "7-4", "7-5"))


# Creating Table
tab_df(Post_Hoc,title = "Table 4: Post-Hoc Aphid Shoots Plum by Treatment",digits = 3)
...
error_bars_aphid_shoots_plum <- ggplot(Aphid_shoots_plum,aes(treatment,aphid_shoots))+
  ...
  theme_bw()+ylab("Mean number of Aphid Shoots");error_bars_aphid_shoots_plum
#Save plot
ggsave("error_bars_aphid_shoots_plum_1.1.tiff", units="in", width=10, height=5, dpi=300, compression = 'lzw')
```


From the visual comparison, it can be observed that the treatment `5` has the higher value Aphid Shoots Plum as compared to all other treatments. Similarly, all the other differences can be asses by observing the visualization. 
...
#Including treatment 1,2,4,5,6,7
#Filtering  Replica #5 for treatment 1 and 2
#Filtering Replica #3 and treatment 6
Aphid_population_plum  <- Aphid_population_plum  %>%
  ...
  Aphid_shoots_time_series2 <- Aphid_population_plum %>% 
  mutate(treatment=dplyr::recode(treatment, '1' = "Control", '2' = "FS14", '4' = "Control+Aphid", '5' = "FS14+Aphid", '6' = "FS1", '7' = "FS1+Aphid" )) 

#Below is the script for manipulating the data for use in Minitab for creating the time series graph. 

timeseries_aphid_shoots_plum2 <- data.frame(Aphid_shoots_time_series2 %>%
                                  group_by(treatment, date) %>%
...
p2 <- ggplot(timeseries_aphid_shoots_plum2) +
 aes(x = date, y = Mean, colour = treatment, group = treatment, ) +
 geom_line(size = 0.5) +
 geom_point()+
 scale_colour_manual(values=cbp1) +
 labs(x = "Week", y = "Mean population of H. Pruni") +
 theme_bw()
...
#write.xlsx(Aphid_population_plum, "spray_frass__population_plum")
...
tab_df(aphid_population_mean,
       title = "Table 5: Descriptive Statistics of Aphid Population on Plum trees in relation to Treatment")
```
...
P.adj <- c(1.000000000000000000000000000, 0.000000000000000000247233569, 0.000000000000000000000000177, 0.000000000000000000000000125, 0.000000000000000000002121168, 0.000000000000000000247627867, 0.000000000000000000000000177, 0.000000000000000000000000126, 0.000000000000000000002124734, 1.000000000000000000000000000, 0.505205508010164061616364961, 1.000000000000000000000000000, 1.000000000000000000000000000, 1.000000000000000000000000000, 1.000000000000000000000000000)
...
# Creating Table
tab_df(Post_Hoc,title = "Table 6: Post-Hoc of Aphid Population by Treatment",digits = 3)
...
error_bars_Aphid_Population_plum <- ggplot(Aphid_population_plum,aes(treatment,aphid_population))+
  geom_bar(fill="darkblue",stat = "summary",fun="mean",col="red")+xlab("Treatment")+
  geom_errorbar(stat='summary', width=0.8,position = "dodge",col="red")+
  theme_bw()+ylab("Mean Population count of Aphids");error_bars_Aphid_Population_plum
#Save plot
...
## 3: Comparing the Aphid Mites Plum for each treatment - for sprayed frass and soil
...
Mites_plum  <- Mites_plum %>%
  filter(replica != 3 | treatment != 6) %>% 
  select(treatment, total_mites) %>% 
  mutate(treatment = as_factor(treatment)) 

levels(Mites_plum$treatment)[levels (Mites_plum$treatment)=="1"] <-"No_treatment"
levels(Mites_plum$treatment)[levels (Mites_plum$treatment)=="4"] <-"No_treatment"
levels(Mites_plum$treatment)[levels (Mites_plum$treatment)=="2"] <-"FS14"
levels(Mites_plum$treatment)[levels (Mites_plum$treatment)=="5"] <-"FS14"
levels(Mites_plum$treatment)[levels (Mites_plum$treatment)=="6"] <-"FS1"
levels(Mites_plum$treatment)[levels (Mites_plum$treatment)=="7"] <-"FS1"
levels(Mites_plum$treatment)[levels (Mites_plum$treatment)=="8"] <-"No_treatment"
levels(Mites_plum$treatment)[levels (Mites_plum$treatment)=="1"] <-"No_treatment"
levels(Mites_plum$treatment)[levels (Mites_plum$treatment)=="9"] <-"FT"
levels(Mites_plum$treatment)[levels (Mites_plum$treatment)=="3"] <-"FT"
...
#Reporting results
noquote("Results of Kruskal-Wallis rank sum test")
cat("Kruskal-Wallis chi-squared =",6,"\n")
cat("Degree of Freedom =",5,"\n")
cat("P-value <","0.1","\n")
```
...
#creating vector of p.adjusted so that it fits the arrangement of Mean_Diff
P.adj <- c(1.000, 0.215, 1.000, 0.262, 1.000, 1.000	)
#P.adj <- c(1.0000, 1.0000, 0.4772, 0.0810, 1.0000, 1.0000, 0.0913, 0.0127, 1.0000, 1.0000, 0.3748, 1.0000, 1.0000, 1.0000, 0.2200)
...
error_bars_total_mites_plum <- ggplot(Mites_plum,aes(treatment,total_mites))+
  geom_bar(fill="darkblue",stat = "summary",fun="mean",col="red")+xlab("Treatment")+
  geom_errorbar(stat='summary', width=0.8,position = "dodge",col="red")+
  theme_bw()+ylab("Total Mite population");error_bars_total_mites_plum
#Save plot
...
Aphid_shoots_plum  <- Aphid_shoots_plum  %>%
  ...
  Aphid_shoots_time_series3 <- Aphid_shoots_plum %>% 
  mutate(treatment=dplyr::recode(treatment, '1' = "Control", '3' = "FSoil", '8' = "Control+Aphid", '9' = "FSoil+Aphid")) 

#Below is the script for manipulating the data for use in Minitab for creating the time series graph. 

timeseries_aphid_shoots_plum3 <- data.frame(Aphid_shoots_time_series3 %>%
                                  group_by(treatment, date) %>%
                                  summarise(Mean = mean(aphid_shoots)))
...
p3 <- ggplot(timeseries_aphid_shoots_plum3) +
 aes(x = date, y = Mean, colour = treatment, group = treatment, ) +
 geom_line(size = 0.5) +
 geom_point()+
 scale_colour_manual(values=cbp1) +
...
#exporting for use in minitab in glm analyses
#write.xlsx(Aphid_shoots_plum, "Frass_in_soil_shoots")
...
Aphid_population_plum <- Aphid_population_plum  %>%
 ...
 Aphid_shoots_time_series4 <- Aphid_population_plum %>% 
  mutate(treatment=dplyr::recode(treatment, '1' = "Control", '3' = "FSoil", '8' = "Control+Aphid", '9' = "FSoil+Aphid")) 

#Below is the script for manipulating the data for use in Minitab for creating the time series graph. 

timeseries_aphid_shoots_plum4 <- data.frame(Aphid_shoots_time_series4 %>%
                                  group_by(treatment, date) %>%
                                  summarise(Mean = mean(aphid_population)))
...
p4 <- ggplot(timeseries_aphid_shoots_plum4) +
 aes(x = date, y = Mean, colour = treatment, group = treatment, ) +
 geom_line(size = 0.5) +
 geom_point()+
 scale_colour_manual(values=cbp1) +
...
#exporting for use in minitab for glm analyses
#write.xlsx(Aphid_population_plum, "Frass_in_soil_population")
```
...
#Below is the script for manipulating the data for use in Minitab for creating the time series graph.
#minitab_4 <- data.frame(Aphid_population_plum %>%
#                                 group_by(treatment, date) %>%
#                                 summarise(Mean = mean(aphid_population)))

#write.xlsx(minitab_4, "for_minitab4")
```
...
#Filtering out unrelevant pairingss
#Post_Hoc <- Post_Hoc %>% 
#  filter(Pairs %in% c("3-1", "9-8"))
# Creating Table
...
## 3: Comparing the Aphid Mites Plum for each treatment !!! Redone together with sprayed frass - keeping this in case I need to redo
...
#creating vector of p.adjusted so that it fits the arrangement of Mean_Diff
P.adj <- c(0.18064, 0.00958,	0.00858, 1.00000, 1.00000, 1.00000)
#Creating table of results
Post_Hoc <- cbind(Mean_Diff, P.adj = P.adj)
# Creating Table
tab_df(Post_Hoc,title = "Table 6: Post-Hoc Total Mites Plum by Treatment",digits = 3)
